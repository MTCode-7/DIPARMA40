...
<title><?= dp_t('Withdrawal database', 'قاعدة بيانات السحب') ?> | DI PARMA</title>
...
<header class="top"><div class="brand"><?= dp_t('Withdrawal database', 'قاعدة بيانات السحب') ?><small>Withdrawal Operations Database</small></div><a class="back" href="dashboard.php"><?= dp_t('Dashboard', 'لوحة التحكم') ?> ←</a></header>
<section class="panel"><h1 style="margin:0;font-size:1.25rem"><?= dp_t('All withdrawal operations', 'كل عمليات السحب') ?></h1><div class="muted"><?= dp_t('Full operation numbers and IDs are shown. Card data and CVV are protected.', 'كل أرقام العمليات والمعرّفات التشغيلية تظهر كاملة. بيانات البطاقة وCVV محمية.') ?></div><div class="stats"><div class="stat"><b><?=number_format((int)($stats['total'] ?? 0))?></b><span><?= dp_t('Total operations', 'إجمالي العمليات') ?></span></div><div class="stat"><b><?=number_format((float)($stats['amount'] ?? 0),2)?></b><span><?= dp_t('Total amounts', 'إجمالي المبالغ') ?></span></div><div class="stat"><b><?=number_format((int)($stats['completed'] ?? 0))?></b><span><?= dp_t('Successful', 'ناجحة') ?></span></div><div class="stat"><b><?=number_format((int)($stats['failed'] ?? 0))?></b><span><?= dp_t('Failed or cancelled', 'فاشلة أو ملغاة') ?></span></div></div></section>
<section class="panel"><form class="filters" method="get"><div class="field"><label><?= dp_t('Search', 'بحث') ?></label><input name="search" value="<?=wdEscape($search)?>" placeholder="<?= dp_t('Reference, RRN, Approval, gateway...', 'مرجع، RRN، Approval، بوابة...') ?>"></div><div class="field"><label><?= dp_t('Status', 'الحالة') ?></label><select name="status"><option value=""><?= dp_t('All statuses', 'كل الحالات') ?></option><?php foreach(['completed','pending','processing','failed','declined','cancelled','refunded'] as $option): ?><option value="<?=$option?>" <?=$status===$option?'selected':''?>><?=$option?></option><?php endforeach; ?></select></div><div class="field"><label><?= dp_t('From date', 'من تاريخ') ?></label><input type="date" name="date_from" value="<?=wdEscape($dateFrom)?>"></div><div class="field"><label><?= dp_t('To date', 'إلى تاريخ') ?></label><input type="date" name="date_to" value="<?=wdEscape($dateTo)?>"></div><button class="btn"><?= dp_t('Search', 'بحث') ?></button></form></section>
<section class="panel"><div class="table-wrap"><table class="table"><thead><tr><th><?= dp_t('Date', 'التاريخ') ?></th><th><?= dp_t('Reference', 'المرجع') ?></th><th><?= dp_t('Operation type', 'نوع العملية') ?></th><th><?= dp_t('Amount', 'المبلغ') ?></th><th><?= dp_t('Status', 'الحالة') ?></th><th><?= dp_t('Card', 'البطاقة') ?></th><th>RRN</th><th>Approval</th><th><?= dp_t('Gateway', 'البوابة') ?></th><th><?= dp_t('All data', 'كل البيانات') ?></th></tr></thead><tbody><?php if (!$rows): ?><tr><td colspan="10" style="text-align:center;padding:40px;color:var(--muted)"><?= dp_t('No withdrawal operations.', 'لا توجد عمليات سحب.') ?></td></tr><?php endif; ?><?php foreach($rows as $row): $state=strtolower((string)($row['status']??'unknown')); $stateClass=in_array($state,['completed','approved','captured','settled'],true)?'ok':(in_array($state,['failed','declined','cancelled','refunded'],true)?'bad':'wait'); $json=[]; foreach(['gateway_response','transaction_data'] as $column){if(!empty($row[$column])){$decoded=json_decode((string)$row[$column],true);$json[$column]=is_array($decoded)?$decoded:$row[$column];}} ?><tr><td class="mono"><?=wdEscape($row['created_at']??'—')?></td><td class="mono"><?=wdEscape($row['reference']??'—')?></td><td><?=wdEscape($row['transaction_label']??$row['transaction_type']??'—')?><br><span class="muted"><?=wdEscape($row['transaction_type']??'')?></span></td><td class="mono"><?=number_format((float)($row['amount']??0),2)?> <?=wdEscape($row['currency']??'')?></td><td><span class="status <?=$stateClass?>"><?=wdEscape(strtoupper($state))?></span></td><td><?=wdEscape($row['card_brand']??'')?> ****<?=wdEscape(substr((string)($row['card_last4']??''),-4))?><br><span class="muted"><?=wdEscape($row['cardholder_name']??'')?></span></td><td class="mono"><?=wdEscape($row['rrn']??'—')?></td><td class="mono"><?=wdEscape($row['approval_code']??$row['auth_code']??'—')?></td><td><?=wdEscape($row['gateway']??'—')?><br><span class="muted"><?=wdEscape($row['gateway_type']??'')?></span></td><td><details class="details"><summary><?= dp_t('View data', 'عرض البيانات') ?></summary><div class="all-data"><?php foreach($row as $key=>$value): if(in_array($key,['gateway_response','transaction_data'],true))continue; ?><div><b><?=wdEscape(wdLabel((string)$key))?></b><span><?=wdEscape(wdSafeValue((string)$key,$value))?></span></div><?php endforeach; ?><?php foreach($json as $key=>$value): ?><div style="grid-column:1/-1"><b><?=wdEscape(wdLabel($key))?></b><span><?=wdEscape(wdSafeValue($key,$value))?></span></div><?php endforeach; ?></div></details></td></tr><?php endforeach; ?></tbody></table></div><?php if($totalPages>1): ?><div class="pager"><?php if($page>1): ?><a href="<?=wdQueryUrl($page-1)?>"><?= dp_t('Previous', 'السابق') ?></a><?php endif; ?><?= dp_t('Page', 'صفحة') ?> <?=$page?> <?= dp_t('of', 'من') ?> <?=$totalPages?><?php if($page<$totalPages): ?><a href="<?=wdQueryUrl($page+1)?>"><?= dp_t('Next', 'التالي') ?></a><?php endif; ?></div><?php endif; ?></section>
</main></body></html>
