...
<section class="panel"><h2>Payment Operation Modes</h2><div class="capabilities"><?php foreach ($capabilities as $label => $supported): ?><div class="cap"><span><?= $escape($label) ?></span><strong class="<?= $supported ? 'yes' : 'no' ?>"><?= $supported ? 'Available' : 'Unavailable' ?></strong></div><?php endforeach; ?></div><?php if ($code === 'square'): ?><p class="muted">Square Offline is Square POS / Terminal / Register / Handheld store-and-forward — not DIPARMA keyed entry. Square does not support manual PAN, Tap to Pay, gift cards, Afterpay, Cash App Pay, or EBT while offline. Allow it in Dashboard <?= $escape((string)($offlineGuide['dashboard_path'] ?? 'Settings > Device Management > Modes > Manage > Offline payments')) ?> or POS <?= $escape((string)($offlineGuide['pos_path'] ?? 'More > Settings > Checkout > Offline payments')) ?>. Take and upload within 24 hours; pending payments expire after 72 hours and cannot be recovered. Per-transaction max: 50,000 USD. While pending: do not sign out, delete the app, switch mode/location, or factory reset. Refunds only after upload. From April 2026 Square may auto-enable offline on all devices unless you keep existing settings. DIPARMA Web Payments still needs connect.squareup.com. After Square captures online: net USDT → Ledger.</p><?php else: ?><p class="muted">Capabilities are derived from the gateway configuration and recorded transaction fields.</p><?php endif; ?></section>
</div>
