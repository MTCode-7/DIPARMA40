...
<section class="panel"><h2>Payment Operation Modes</h2><div class="capabilities"><?php foreach ($capabilities as $label => $supported): ?><div class="cap"><span><?= $escape($label) ?></span><strong class="<?= $supported ? 'yes' : 'no' ?>"><?= $supported ? 'Available' : 'Unavailable' ?></strong></div><?php endforeach; ?></div><?php if ($code === 'square'): ?><p class="muted">Square Offline is the Square POS / hardware store-and-forward (guide GTM00173). Card charges sit on the Square device, then upload when the device is back online. Reconnect within 72 hours (Square recommends 24). Per-transaction offline max in this account catalog: 50,000 USD. POS: <?= $escape((string)($offlineGuide['pos_path'] ?? 'More > Settings > Checkout > Offline payments')) ?>. Dashboard: <?= $escape((string)($offlineGuide['dashboard_path'] ?? 'Settings > Device Management > Modes > Manage > Offline payments')) ?>. During a session keep Wi-Fi/ethernet on, do not switch mode, location, or sign out. Merchant is liable for expired, declined, or disputed offline payments. DIPARMA Web Payments API charges still need a live connection to connect.squareup.com.</p><?php else: ?><p class="muted">Capabilities are derived from the gateway configuration and recorded transaction fields.</p><?php endif; ?></section>
</div>
