...
$transactions = $db->query(
    'SELECT id, reference, amount, currency, transaction_type, status, created_at, gateway_response FROM ' . DB_PREFIX . 'transactions WHERE gateway = ? AND ' . diparma_visible_transaction_sql() . ' ORDER BY created_at DESC LIMIT 10',
    [$code]
);
...
$limitLabel = $maxPerTxn !== null
    ? number_format($maxPerTxn, 2) . ' USD / txn' . ($maxOffline !== null ? ' · offline ' . number_format($maxOffline, 2) . ' USD' : '')
    : 'No per-transaction cap in catalog';
$offlineGuide = (array)($catalog['offline_guide'] ?? $mergedConfig['offline_guide'] ?? []);
...
<div class="layout">
<section class="panel"><h2>Gateway Information</h2><div class="meta"><span>Type</span><strong><?= $escape($gateway['type'] ?? 'Payment Gateway') ?></strong></div><div class="meta"><span>Connection</span><strong><?= $escape($gateway['connection_type'] ?? 'REST') ?></strong></div><div class="meta"><span>Endpoint</span><strong><?= $escape($endpoint ?: 'Unavailable') ?></strong></div><div class="meta"><span>Currencies</span><strong><?= $escape(implode(', ', (array)($mergedConfig['currencies'] ?? [])) ?: 'Unavailable') ?></strong></div><div class="meta"><span>Limits</span><strong><?= $escape($limitLabel) ?></strong></div><div class="meta"><span>Features</span><strong><?= $escape(implode(', ', $features) ?: 'Unavailable') ?></strong></div></section>
<section class="panel"><h2>Payment Operation Modes</h2><div class="capabilities"><?php foreach ($capabilities as $label => $supported): ?><div class="cap"><span><?= $escape($label) ?></span><strong class="<?= $supported ? 'yes' : 'no' ?>"><?= $supported ? 'Available' : 'Unavailable' ?></strong></div><?php endforeach; ?></div><?php if ($code === 'square'): ?><p class="muted">Square Offline is Square POS / Terminal / Register / Handheld / Stand / Reader store-and-forward — not DIPARMA keyed entry. Square does not support manual PAN, Tap to Pay, gift cards, Afterpay, Cash App Pay, EBT, Invoices app, POS services mode, Kiosk, or Terminal API buyer-facing while offline. Allow it in Dashboard <?= $escape((string)($offlineGuide['dashboard_path'] ?? 'Settings > Device management > Modes > Manage > Checkout settings > Offline payments')) ?> or POS <?= $escape((string)($offlineGuide['pos_path'] ?? 'More > Settings > Checkout > Offline payments')) ?>. Readers/Stand: 24 hours to take; Register/Terminal/Handheld: upload within 72 hours of the session (24 hours recommended). Pending expire after 72 hours and cannot be recovered. Per-transaction max: $1–50,000. While pending: do not sign out, delete the app, switch mode/location, or factory reset. Refunds only after upload. From April 2026 Square may auto-enable offline on all devices unless you keep existing settings. DIPARMA Web Payments still needs connect.squareup.com. After Square captures online the funds stay on Square.</p>
<p class="muted">View pending only in the Square POS app (Transactions or More &gt; Transactions — circle with arrows). DIPARMA and Square Dashboard cannot show pending offline payments. After reconnect/upload: completed in Dashboard Orders &amp; payments &gt; Transactions (Complete + Offline payment note) or Orders; declined as a red note on Orders or Reports &gt; Payments &gt; Discounts named Offline Declined Payment. No decline alerts while offline; the seller is liable for expired, declined, or disputed offline payments. <a href="<?= $escape((string)($offlineGuide['help_url'] ?? 'https://squareup.com/help/us/en/article/7777-process-card-payments-with-offline-mode')) ?>" target="_blank" rel="noopener">Process offline payments</a> · <a href="<?= $escape((string)($offlineGuide['view_url'] ?? 'https://squareup.com/help/us/en/article/8551-view-offline-payments')) ?>" target="_blank" rel="noopener">View offline payments</a> · <a href="<?= $escape((string)($offlineGuide['status_url'] ?? 'https://issquareup.com')) ?>" target="_blank" rel="noopener">issquareup.com</a></p><?php else: ?><p class="muted">Capabilities are derived from the gateway configuration and recorded transaction fields.</p><?php endif; ?></section>
</div>
<section class="panel" style="margin-top:16px"><h2>Last 10 Transactions</h2><?php if (!$transactions): ?><p class="muted">No transactions recorded for this gateway.</p><?php else: ?><table><thead><tr><th>Reference</th><th>Amount</th><th>Type</th><th>Status</th><th>Hang reason</th><th>Created</th></tr></thead><tbody><?php foreach ($transactions as $transaction): $hang = function_exists('diparma_transaction_hang_reason') ? diparma_transaction_hang_reason($transaction) : []; ?><tr><td><?= $escape($transaction['reference']) ?></td><td><?= $escape(number_format((float)$transaction['amount'], 2) . ' ' . $transaction['currency']) ?></td><td><?= $escape($transaction['transaction_type'] ?: 'Purchase') ?></td><td><?= $escape($transaction['status']) ?></td><td><?= $escape(($hang['en'] ?? '') . (!empty($hang['live_status']) ? ' [' . $hang['live_status'] . ']' : '')) ?></td><td><?= $escape($transaction['created_at']) ?></td></tr><?php endforeach; ?></tbody></table><?php endif; ?></section>
</main>
</body>
</html>
