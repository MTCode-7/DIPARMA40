...
$destination=$_GET['destination']??'gateway'; $ref=$_GET['ref']??('GIO-'.strtoupper(substr(uniqid(),0,8)));
$walletAddr=$_GET['wallet']??'TEwLFWlwK55b7PuFfzgH1H2f3xs3pLgLn2';
?><!DOCTYPE html>
<html lang="<?=$lang?>" dir="<?=$dir?>">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>DI PARMA | Gate.io Pay</title>
<link href="https://fonts.googleapis.com/css2?family=Cairo:wght@400;600;700;800;900&display=swap" rel="stylesheet">
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
<style>
*{margin:0;padding:0;box-sizing:border-box}:root{--gold:#FFD700;--bg:#030609;--card:#090f1e;--border:rgba(255,215,0,.12);--border2:rgba(255,215,0,.28);--text:#edf0f7;--muted2:#718096;--green:#10B981;--red:#EF4444;--bnb:#E8112D}
...
<body>
<header class="topbar">
  <div class="tb-brand"><i class="fas fa-coins"></i> DI PARMA <span style="color:var(--muted2)">|</span><span style="background:rgba(243,186,47,.1);border:1px solid rgba(243,186,47,.2);border-radius:8px;padding:4px 12px;font-size:.72rem;font-weight:800;color:var(--bnb)"><i class="fas fa-coins"></i> Gate.io</span></div>
  ...
  try{
    const r=await fetch('../api/pos_transaction.php',{method:'POST',headers:{'Content-Type':'application/json'},
      body:JSON.stringify({txn_type:'purchase',amount:AMOUNT,currency:'USDT',destination:DESTINATION,
        reference:REF,pos_device:'Gate.io_CRYPTO',csrf_token:CSRF,
