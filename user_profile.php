...
<title>DI PARMA | <?= dp_t('User profile', 'ملف المستخدم') ?></title>
...
<div class="container">
  ...
  <div class="card">
    <h2><?= dp_t('User profile', 'ملف المستخدم') ?></h2>
    <div class="grid">
      <div class="item"><div class="label"><?= dp_t('Username', 'اسم المستخدم') ?></div><div class="value"><?= htmlspecialchars($user['username'] ?? '-') ?></div></div>
      <div class="item"><div class="label"><?= dp_t('Email', 'البريد') ?></div><div class="value"><?= htmlspecialchars($user['email'] ?? '-') ?></div></div>
      <div class="item"><div class="label"><?= dp_t('Full name', 'الاسم الكامل') ?></div><div class="value"><?= htmlspecialchars(trim(($user['first_name'] ?? '') . ' ' . ($user['last_name'] ?? ''))) ?></div></div>
      <div class="item"><div class="label"><?= dp_t('Role', 'الدور') ?></div><div class="value"><?= htmlspecialchars($user['role'] ?? 'user') ?></div></div>
    </div>
  </div>
  <div class="card">
    <h3><?= dp_t('Recent transactions', 'آخر المعاملات') ?></h3>
    <table>
      <thead><tr><th><?= dp_t('Reference', 'المرجع') ?></th><th><?= dp_t('Amount', 'المبلغ') ?></th><th><?= dp_t('Status', 'الحالة') ?></th><th><?= dp_t('Date', 'التاريخ') ?></th></tr></thead>
      <tbody>
  ...
  <div class="card">
    <h3><?= dp_t('Wallets', 'المحافظ') ?></h3>
    <table>
      <thead><tr><th><?= dp_t('Currency', 'العملة') ?></th><th><?= dp_t('Balance', 'الرصيد') ?></th><th><?= dp_t('Status', 'الحالة') ?></th></tr></thead>
      <tbody>
